...
class afficheGrid(wx.grid.Grid):
    ...
    def __init__(self, parent,table=None,db='',key='n_enr',idcom=0,nomcommande=0,wdgchecksta=None,where='',tchampvue=[],isctrlann=True,pos=(10,10)):
        ...
        #self._table = MegaFontRenderer(self._table)
        self.SetTable(self._table)
        self._plugins = None

        #self.font = wx.Font(10, wx.DEFAULT, wx.NORMAL, wx.NORMAL, 0, 'ARIAL')
        ...
        nl=0
#
#        for nl in range(nbLigne):
#            for nj in range(nbCols):
#                self.SetReadOnly(nl,nj)


        #while (nl<nbLigne):
        #    nj=0
        #    while(nj<nbCols):
        #        self.SetCellFont(nl,nj, wx.Font(12, wx.SWISS, wx.NORMAL, wx.NORMAL))
        #        nj = nj+1
        #    nl=nl+1
        ...
        #self.Bind(wx.grid.EVT_GRID_LABEL_RIGHT_CLICK,self.gotoContxMenu)
        ...
        #self.Bind(wx.EVT_GRID_COL_SORT ,self.sortir)
...
#                print 'mot entier avec casse'
    ...
    def changer(self,e,ln=None,col=None):
        ...
        #print "UPDATE \"" +str(self.table)+ "\" SET \""+str(self.tableColumn[cols])+"\"='"+str(new).replace("'","\\'").replace("\\","\\\\")+"' WHERE \""+str(self.key)+"\" = '"+str(id)+"'"
        #try:
        try:
            self.curseur.execute("UPDATE \"" +str(self.table)+ "\" SET   \""+str(self.tableColumn[cols])+"\"='"+str(new).replace("'","''")+"' WHERE \""+str(self.key)+"\" = '"+str(id)+"'")
        except Exception as inst:
            msgs = 'Modification echouée:\n'
    ...
    def opens(self,e):
        ...
        nima = self.GetCellValue(e.GetRow(),self.getColImage("n_ima"))
...
#            print servimage_abs
        openimg.MainWindow(servimage_abs,lot,pli=pli,n_ima=nima)
        obj.SetFocus()
    ...
    def opens0(self,e):
        ...
        #curlocal = self.connexion.cursor(cursor_factory=psycopg2.extras.DictCursor)
        #curlocal.execute("SET client_encoding = 'WIN1252';select * from export where \"N_IMA\"='"+str(img)+"' and \"N_LOT\"='"+str(lot)+"'")
        #datas = curlocal.fetchone()
        #if datas!=None:
            #pli = datas['LIST_IMA']
        #else:
            #pli=''
        #openimg.MainWindow(self.servimage+self.repserveur+self.GetCellValue(e.GetRow(),6)+'/'+self.GetCellValue(e.GetRow(),1))
#        print len(timg)
